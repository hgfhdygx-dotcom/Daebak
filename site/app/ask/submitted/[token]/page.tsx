import type { Metadata } from "next";
import Link from "next/link";
import LineIcon from "@/components/LineIcon";
import { getPublicQuestion } from "@/lib/questions";

export const dynamic = "force-dynamic"; // token 별 서버 렌더(캐시/정적화 안 함)
export const metadata: Metadata = {
  title: "Your question · Daebak",
  robots: { index: false, follow: false },
};

const STATUS_COPY: Record<string, { label: string; text: string; tone: string }> = {
  new: { label: "Received", text: "We received your question.", tone: "text-ink-muted" },
  reviewing: { label: "Reviewing", text: "Daebak is reviewing this question.", tone: "text-accent-ink" },
  selected: { label: "Selected", text: "Daebak selected your question — an answer is on the way.", tone: "text-accent-ink" },
  draft_created: { label: "In progress", text: "An answer is being prepared.", tone: "text-accent-ink" },
  answered: { label: "Answered", text: "We answered your question.", tone: "text-trust" },
  published: { label: "Published", text: "We answered your question.", tone: "text-trust" },
  rejected: { label: "Closed", text: "We couldn't take this one for now.", tone: "text-ink-muted" },
};

function NotFound() {
  return (
    <div className="mx-auto max-w-[560px] px-5 py-16 text-center sm:px-8">
      <h1 className="font-display text-2xl font-bold tracking-tight">No question found</h1>
      <p className="mt-3 text-sm leading-relaxed text-ink-muted">
        We couldn&apos;t find an active question for this link. The link may be incorrect or no longer available.
      </p>
      <Link href="/ask" className="mt-6 inline-flex items-center gap-1.5 text-sm font-semibold text-accent-ink">
        Ask a new question
        <LineIcon name="arrow-right" className="h-4 w-4" />
      </Link>
    </div>
  );
}

export default async function SubmittedPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const q = await getPublicQuestion(token);
  if (!q || q.status === "spam") return <NotFound />;

  const copy = STATUS_COPY[q.status] || STATUS_COPY.new;
  const submitted = (() => {
    try {
      return new Date(q.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" });
    } catch {
      return "";
    }
  })();
  const answered = q.status === "published" || q.status === "answered";

  return (
    <div className="mx-auto max-w-[640px] px-5 py-12 sm:px-8 lg:py-16">
      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-muted">
        Thanks for asking Daebak{q.displayId ? ` · ${q.displayId}` : ""}
      </p>

      <div className="mt-4 rounded-2xl border border-line bg-surface p-5 sm:p-6">
        <p className="font-display text-lg font-bold leading-snug tracking-tight text-ink">{q.question}</p>
        {submitted ? <p className="mt-2 text-xs text-ink-soft">Submitted {submitted}</p> : null}

        <div className="mt-4 border-t border-line pt-4">
          <p className={`inline-flex items-center gap-1.5 text-sm font-semibold ${copy.tone}`}>
            <LineIcon name={answered ? "check" : "clock"} className="h-4 w-4" strokeWidth={2.25} />
            {copy.label}
          </p>
          <p className="mt-1.5 text-sm leading-relaxed text-ink-muted">{copy.text}</p>

          {q.answerSummary ? (
            <div className="mt-4 rounded-xl border border-line bg-bg p-4">
              <p className="text-[0.7rem] font-semibold uppercase tracking-[0.12em] text-accent-ink">Daebak&apos;s answer</p>
              <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-ink">{q.answerSummary}</p>
            </div>
          ) : null}

          {q.isPublic && q.publicSlug ? (
            <Link
              href={`/ask/${q.publicSlug}`}
              className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-accent-hover"
            >
              View public question
              <LineIcon name="arrow-right" className="h-4 w-4" />
            </Link>
          ) : answered && q.publishedUrl ? (
            <Link
              href={q.publishedUrl}
              className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-accent-hover"
            >
              Read the full guide
              <LineIcon name="arrow-right" className="h-4 w-4" />
            </Link>
          ) : null}
        </div>
      </div>

      <p className="mt-5 text-xs leading-relaxed text-ink-soft">
        Your question is private. If Daebak selects it, it may become a public Daebak question. Your email
        will never be shown.
      </p>
      <Link href="/ask" className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-accent-ink">
        Ask another question
        <LineIcon name="arrow-right" className="h-4 w-4" />
      </Link>
    </div>
  );
}
